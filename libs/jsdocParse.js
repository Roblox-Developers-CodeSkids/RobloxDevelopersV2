/*
Credit: https://github.com/Geo1088/eris-docs-bot/blob/master/misc/jsdoc_parse.js
*/
...
for (const doclet of rawDocsData) {
  ...
  if (doclet.undocumented) {
    if (
      doclet.meta.filename !== 'Constants.js' ||
  ...
  switch (doclet.kind) {
    case 'class': {
      const baseParams =
        doclet.params &&
        doclet.params.filter((p) => !p.name.includes('.')).map(paramPropMapper);
      const classObject = {
        name: doclet.name,
    ...
    case 'member': {
      ...
      const baseParams =
        params &&
        params.filter((p) => !p.name.includes('.')).map(paramPropMapper);
      const obj = {
        name: doclet.name,
        kind: doclet.kind,
        display: doclet.longname,
        ...
        returns:
          doclet.returns &&
          doclet.returns.map((r) => r.type.names.join(' | ')).join(', '),
      };
    ...
    default: {
      logger.warn('Warning: Unknown doclet kind:', doclet.kind);
    }
  }
}

logger.info(`Loaded documentation for eris@${erisPackage.version}`);
