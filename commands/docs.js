/*
Source: https://github.com/Geo1088/eris-docs-bot/blob/master/commands/docs.js 
*/
...
const { classes, constants } = require('../libs/jsdocParse');
...
const embedDefaults = {
  color: 0x4e98d8,
  footer: {
    text: `Docs generated from eris@${erisPackage.version} | ,docs Class#property`,
  },
};
...
function handleResults(results) {
    ...
    return {
      ...
      embed: {
        ...
        description: results
          .map((result) => {
            if (result.item.kind) {
              return `[\`${result.item.display}\`](${linkTo(
                result.cls.name,
                result.item
              )})`;
            }
            return `[\`${result.cls.name}#${result.item.name}\`](${linkTo(
              result.cls.name,
              result.item,
              'property'
            )})`;
  ...
  return {
    ...
    embed: {
      ...
      description: `\`${results
        .slice(0, 6)
        .map((result) => result.item.display)
        .join('`, `')}\`, and ${results.length - 6} more.`,
      title: 'Multiple Results Found',
    },
  };
}
...
module.exports = new Command(['docs', ''], (msg, args) => {
  ...
  if (!classname) {
    return msg.channel
      .createMessage({
        ...
        embed: {
          ...embedDefaults,
          description: classes
            .map((cls) => `[${cls.name}](${linkTo(cls.name)})`)
            .join(', '),
          title: 'Classes',
        },
      })
    ...
    const docsItem = constants.find(
      (constant) => constant.name.toLowerCase() === search.toLowerCase()
    );
    if (docsItem) {
      return msg.channel
        .createMessage(formatConstant(docsItem))
        .catch(console.error);
    }
    return msg.channel
      .createMessage({
  ...
  const docsClass = classes.find(
    (cls) => cls.name.toLowerCase() === classname.toLowerCase()
  );
  if (!docsClass) {
    let items = constants.find(
      (constant) => constant.name.toLowerCase() === classname.toLowerCase()
    );
    if (items) {
      return msg.channel
        .createMessage(formatConstant(items))
        .catch(console.error);
    }
    items = [];
    for (const cls of classes) {
      for (const category of memberCategories) {
        const item =
          cls[category] &&
          cls[category].find(
            (i) => i.name.toLowerCase() === classname.toLowerCase()
          );
    ...
    if (items.length) {
      return msg.channel
        .createMessage(handleResults(items))
        .catch(console.error);
    }
    return msg.channel
      .createMessage({
  ...
  for (const category of memberCategories) {
    const item =
      docsClass[category] &&
      docsClass[category].find(
        (thing) => thing.name.toLowerCase() === search.toLowerCase()
      );
